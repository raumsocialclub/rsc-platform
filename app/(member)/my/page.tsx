...
export default async function MyPage({ searchParams }: { searchParams: Promise<{ tab?: string }> }) {
  const [{ tab }, me, { bookings, now }] = await Promise.all([searchParams, getCurrentMember(), listMyBookingsWithNow()]);
  return <BookingList bookings={bookings} tab={tab === "past" ? "past" : "upcoming"} name={me?.name ?? ""} now={now} />;
}
