...
/** 목록 행의 상태 배지 + 공개/비공개 토글 */
export function ProgramRowActions({ id, published, badge }: { id: string; published: boolean; badge: React.ReactNode }) {
  const router = useRouter();
  const [busy, setBusy] = useState(false);
  const toggle = async () => {
    setBusy(true);
    try {
      const res = await fetch(`/api/admin/programs/${id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ is_published: !published }) });
  ...
  return (
    <div className="grid gap-[4px] justify-items-start">
      ...
      <button type="button" onClick={toggle} disabled={busy} className="bg-transparent border-0 p-0 text-[11px] text-[rgba(33,30,25,.5)] cursor-pointer hover:text-brownHover disabled:opacity-50 whitespace-nowrap">
        {busy ? "변경 중…" : published ? "비공개로 전환" : "공개하기"}
      </button>
    </div>
  );
}
